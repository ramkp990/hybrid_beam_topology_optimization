...
class TopologyVAE(nn.Module):
    ...
    def predict_compliance(self, mu):
        return self.compliance_head(mu).squeeze(-1)   # [B]
...
def vae_loss(recon, target, mu, logvar, beta,
    ...
    # ── NEW: compliance regression loss ──────────────────────────────
    comp_loss = torch.tensor(0.0, device=recon.device)
    if w_compliance > 0.0 and c_pred is not None and c_true is not None:
        # normalise compliance to ~[0,1] range using your dataset stats
        # mean ~54, std ~15 from your dataset generation script
...
def main():
    ...
    # ── NEW: load precomputed compliance ─────────────────────────────
    ...
    for epoch in range(EPOCHS):
        ...
        # ── NEW: compliance weight annealing ─────────────────────────
        if epoch < 50:
            w_compliance = 0.0
        else:
