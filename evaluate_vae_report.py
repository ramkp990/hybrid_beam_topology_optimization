...
def vae_loss(recon, target, mu, logvar, beta):
    ...
    bce_bin = binary_loss(recon)

    loss = (recon_loss 
          + beta * kld 
          + 0.5 * vol 
          # thinness removed — fights binary sharpness
          + 3.0 * bce_bin)   # increased from 2.0

    return loss, {
        "recon": recon_loss.item(),
        "kld":   kld.item(),
        "vol":   vol.item(),
        "thin":  0.0,         # keep key for logging compatibility
        "binary": bce_bin.item()
    }
...
# -----------------------------
# NEW: Plotting and Metrics Functions
# -----------------------------
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if cycle_pos < warmup:
            ...
            beta = min(0.04, ((cycle_pos - warmup) / (cycle_len - warmup)) * 0.04)

        model.train()
        breakdown_train = {"recon": 0, "kld": 0, "vol": 0, "thin": 0, "binary": 0}
        breakdown_val   = {"recon": 0, "kld": 0, "vol": 0, "thin": 0, "binary": 0}
        train_loss = 0
        ...
        for x in train_loader:
            ...
            loss, bd = vae_loss(recon, x, mu, logvar, beta)  # ← unpack into bd
            loss.backward()
            opt.step()
            train_loss += loss.item()
            for k in breakdown_train:                         # ← accumulate properly
        ...
        with torch.no_grad():
            for x in val_loader:
                x = x.to(DEVICE)
                recon, mu, logvar = model(x)
                loss_v, bd = vae_loss(recon, x, mu, logvar, beta)  # ← unpack, no .item() on tuple
                val_loss += loss_v.item()                           # ← .item() on the loss tensor
        ...
        if val_loss < best_val and beta >= MIN_BETA_TO_SAVE:
            best_val = val_loss
