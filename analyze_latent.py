...
VAE_PATH         = "dataset/merged_vae_train_report/vae_best.pth"
OUT_DIR          = "analysis/latent_space"
CMAES_RESULT_DIR = "results/densities"   # folder with per-gen .npz files
LATENT_DIM       = 32
DEVICE           = "cuda" if torch.cuda.is_available() else "cpu"
BATCH_SIZE       = 256
# ──────────────────────────────────────────────────────────────────────────
...
if not npz_files:
    ...
    for f in npz_files:
        ...
        if "best_z" in d:
            val = d["best_z"]
            if val.ndim > 0 and val.shape == (32,):
                traj_zs.append(val)
            else:
                print(f"  Skipping {os.path.basename(f)} — best_z is None or wrong shape")
